...
from models.attention import LoRAConv2d
...
def lora_resnet50(pretrained=True, progress=True, r=8, lora_alpha=1.0, **kwargs):
    model = _resnet('resnet50', Bottleneck, [3, 4, 6, 3], pretrained, progress, **kwargs)
    model = apply_lora_to_resnet50(model, r=r, lora_alpha=lora_alpha)
    
    return model

def lora_resnet101(pretrained=True, progress=True, r=8, lora_alpha=1.0, **kwargs):
    model = _resnet('resnet101', Bottleneck, [3, 4, 23, 3], pretrained, progress, **kwargs)
    model = apply_lora_to_resnet50(model, r=r, lora_alpha=lora_alpha)
    
    return model

def apply_lora_to_resnet50(model: ResNet, target_stages: list = ['layer3', 'layer4'], r=8, lora_alpha=1.0):
    """
    Replace specified Conv2d layers in ResNet with LoRAConv2d.
    By default, replace all 1x1 convs in bottlenecks (i.e., conv1 and conv3).
    """

    def replace_conv(module, name_prefix=""):
        for name, child in module.named_children():
            full_name = f"{name_prefix}.{name}" if name_prefix else name
            in_target_stage = any(full_name.startswith(stage) for stage in target_stages)
            if isinstance(child, nn.Conv2d) and child.kernel_size == (1, 1):
                # 只替换 1x1 卷积（可调整条件）
                is_in_downsample = 'downsample' in full_name
                if in_target_stage and not is_in_downsample:
