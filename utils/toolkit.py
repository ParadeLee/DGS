...
def compute_flops(model, input_shape=(1, 3, 224, 224), device='cuda'):
    """
    Compute FLOPs and params of the model.
    Args:
        model: your MANet instance
        input_shape: (B, C, H, W)
        device: 'cuda' or 'cpu'
    """
    model.eval()
    ...
    total_flops = flops.total()  # 返回整数（FLOPs 数量）
